...
NOTE: Operating systems differ in how they treat cloned volumes. VMware ESXi will treat a cloned volume as a volume copy or snapshot volume. The volume will be an available device to use to create a new datastore. For more information on mounting clone volumes and handling snapshot LUNs, see VMware documentation on https://techdocs.broadcom.com/[mounting a VMFS datastore copy^] and https://techdocs.broadcom.com/[managing duplicate VMFS datastores^].
